...
void changeColumn(int column_count, int dir)
{
	...
	digitalWrite(13,HIGH);    // indicate start line following
        ...
        if( (digitalRead(A4)) && (!digitalRead(A3)) && (!digitalRead(A2)) && (!digitalRead(A1)) && (digitalRead(A0)) )
        {
            digitalWrite(13,HIGH);    // indicate start line following
...
////////////////////////////
			switch (next_dir)
			{
				case (east):
					...
					while(!digitalRead(A0));   		//while middle pin is not on white...turn right
					while(digitalRead(A2));   		//while middle pin is not on white...turn right
